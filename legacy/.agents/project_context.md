...
.agents/
  ...
  shims/                      # model-specific rule references, ≤50 lines each
  skills/                     # additional skills (blueprint, etc.)
  architecture/               # optional layer: Mermaid diagram templates + ADR folder
  intents/                    # optional layer: intent template + subdirs
AGENTS.md                     # assembled: global_core + project_context (single source of truth)
CLAUDE.md                     # @AGENTS.md — thin pointer; Claude Code delegates to AGENTS.md
