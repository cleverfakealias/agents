...
SHIM_OUTPUTS_PRESENT    = which pointer files exist (CLAUDE.md, .github/copilot-instructions.md, .cursor/rules/*.mdc, .windsurfrules, global_rules.md)
...
Detection: a shim file `.agents/shims/<model>.md` exists AND no pointer file for that model is present in the repo.

Pointer files are thin single-purpose files (not assembled content) whose presence signals the tool is in use:

| Shim file | Pointer file (presence = "tool in use") |
|---|---|
...
| `.agents/shims/openai.md` | No standard pointer file — always report as `confirm-tool-unused` |
| `.agents/shims/gemini.md` | No standard pointer file — always report as `confirm-tool-unused` |

Note: `init` now includes a self-cleanup step (Phase 3.5) that removes shims for unused tools at setup time. Shims that survive to this point are ones the user kept deliberately, or repos that ran an older version of `init`. Still report and offer removal.

Risk: `confirm-tool-unused`. Absence of the pointer file is a strong signal but not definitive (the user may have a non-standard integration).
