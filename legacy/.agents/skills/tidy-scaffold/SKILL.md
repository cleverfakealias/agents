...
2. **Unused shims** — `.agents/shims/<model>.md` where no corresponding pointer file exists in the repo (no `CLAUDE.md`, no `.github/copilot-instructions.md`, etc.). The `init` skill removes shims for unused tools at setup time, so remaining shims are usually intentional — but this catches misses and repos initialized before that cleanup was added. Requires user confirmation that the tool isn't used.
