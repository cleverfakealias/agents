...
Write a single-line pointer file:
...
> Claude Code reads `CLAUDE.md` before `AGENTS.md`. This one-line file imports `AGENTS.md` via Claude Code's `@filename` syntax — CLAUDE.md is a thin delegate with zero duplicated content. If the user later wants Claude-specific overrides active, they add them above `@AGENTS.md`; see `.agents/shims/claude.md` for a reference set of Claude-specific rules.

### 2D. Generate `.github/copilot-instructions.md` (if requested)

Create `.github/` directory if it doesn't exist.

Write the following pointer text:

```markdown
Agent standards for this repository are in AGENTS.md at the repo root.
Read AGENTS.md before acting on any multi-step task.
```

Write to: `.github/copilot-instructions.md`

> GitHub Copilot reads this file automatically in VS Code. It does not support `@filename` imports, so this is a short text pointer. Copilot has workspace access and will read AGENTS.md directly. Add Copilot-specific rules above the pointer lines if needed; see `.agents/shims/copilot.md` for a reference set.

### 2E. Generate `.cursor/rules/agents.mdc` (if requested)

Create `.cursor/rules/` directory if it doesn't exist.

Write a short pointer file:

```markdown
---
description: Agent standards — read AGENTS.md at the repo root for the full contract.
alwaysApply: true
---

See AGENTS.md at the repo root for complete agent standards and project context.
```

Write to: `.cursor/rules/agents.mdc`

> Cursor reads `AGENTS.md` natively. This scoped rule file exists only to participate in Cursor's `.cursor/rules/*.mdc` system (e.g. to attach via `alwaysApply`). It delegates to `AGENTS.md` rather than duplicating content.

### 2F. Generate Windsurf rules files (if requested)

Write the following pointer text to **both** files:
...
> Windsurf reads `AGENTS.md` natively. These files exist only when the user explicitly relies on Cascade's rule files. Both receive the same pointer text; if the user only wants one, ask which. See `.agents/shims/windsurf.md` for Windsurf-specific rules to add above the pointer if needed.
...
## Step 3.5: Self-Cleanup

Run this immediately after all generation steps complete, before the final report. Do not ask for permission — these are safe, reversible-at-any-time cleanups.

### 3.5A. Remove Consumed Templates (Automatic)

For each template whose generated counterpart now exists, delete the template file without prompting:

| Template to delete | Counterpart that signals it's consumed |
|---|---|
| `.agents/project_context.template.md` | `.agents/project_context.md` |
| `.agents/llms-template.txt` | `llms.txt` (repo root) |

Do **not** delete architecture or intents templates here — those belong to layers the user may not have opted into yet. They remain candidates for `tidy-scaffold` later.

Print one line per deletion: `✗ Removed <path>  (consumed template)`.

### 3.5B. Remove Unused Shims (One Question)

Ask a single multi-select question:

```
Question: "Which AI coding tools does this repo NOT use? I'll remove the shim files for tools you don't use."
Header: "Shim Cleanup"
multiSelect: true
Options:
  1. "Not using Claude Code / claude.ai"
     Description: "Remove .agents/shims/claude.md"
  2. "Not using GitHub Copilot"
     Description: "Remove .agents/shims/copilot.md"
  3. "Not using Cursor"
     Description: "Remove .agents/shims/cursor.md"
  4. "Not using Windsurf"
     Description: "Remove .agents/shims/windsurf.md"
  5. "Not using ChatGPT / OpenAI Codex"
     Description: "Remove .agents/shims/openai.md"
  6. "Not using Gemini"
     Description: "Remove .agents/shims/gemini.md"
```

For each selected option, delete the corresponding shim file. Print one line per deletion: `✗ Removed <path>  (unused shim)`.

If the user selects none (or dismisses), keep all shims and skip silently. The user can clean them up later via `tidy-scaffold`.
...
[if windsurf rules were generated]
  ...
  • global_rules.md  (pointer to AGENTS.md)
...
🧹 Cleaned up:
  [list of files removed in Step 3.5 — consumed templates and unused shims]
  (if nothing was removed: "Nothing to clean — all shims kept.")
...
🚀 Next steps:
  ...
  3. Commit: git add AGENTS.md llms.txt .agents/ && git commit -m "Add agent standards (AGENTS.md, llms.txt)"
  4. (Optional) Set up CI to regenerate AGENTS.md and llms.txt on push if you edit .agents/ files
  5. Run tidy-scaffold later to clean up any remaining .agents/ leftovers (architecture/intents templates, etc.)
```
