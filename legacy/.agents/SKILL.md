...
├── project_context.template.md ← (REMOVED after generation — consumed template)
├── llms-template.txt           ← (REMOVED after generation — consumed template)
├── README.md                   ← (unchanged)
├── SKILL.md                    ← (this file)
├── SKILL-implementation.md     ← (unchanged — execution contract)
└── shims/
    ├── claude.md               ← (kept or REMOVED — depends on your answer to shim cleanup)
    ├── openai.md               ← (kept or REMOVED — depends on your answer to shim cleanup)
    ├── gemini.md               ← (kept or REMOVED — depends on your answer to shim cleanup)
    ├── copilot.md              ← (kept or REMOVED — depends on your answer to shim cleanup)
    ├── cursor.md               ← (kept or REMOVED — depends on your answer to shim cleanup)
    └── windsurf.md             ← (kept or REMOVED — depends on your answer to shim cleanup)

AGENTS.md                       ← (GENERATED: global_core + project_context)
llms.txt                        ← (GENERATED: machine-readable index)
```

Shims are reference documentation for model-specific rules. They are never assembled into generated files — they delegate to `AGENTS.md` instead. After setup you're asked which tools you don't use; shims for unused tools are removed automatically.
...
Always generated (then auto-cleaned):
...
- **Consumed templates removed automatically** — `project_context.template.md` and `llms-template.txt` are deleted after their outputs are written.
- **Unused shims removed on request** — you're asked which AI tools you don't use; their shim files are deleted in the same pass.

Optional model files (only if requested — Cursor and Windsurf already read `AGENTS.md`):
- **`CLAUDE.md`** — Single line: `@AGENTS.md`. Claude Code's import syntax makes this a thin delegate; add Claude-specific overrides above that line if needed. Reference rules in `.agents/shims/claude.md`.
- **`.github/copilot-instructions.md`** — Two-line pointer to `AGENTS.md`. Copilot has workspace access and reads `AGENTS.md` directly. Reference rules in `.agents/shims/copilot.md`.
- **`.cursor/rules/agents.mdc`** — Short pointer with `alwaysApply: true` frontmatter. Generate only if you use Cursor's scoped-rules system.
- **`.windsurfrules`** + **`global_rules.md`** — Short pointer text. Generate only if you use Windsurf's Cascade rule files.

None of these files duplicate `AGENTS.md` content — they delegate to it. `AGENTS.md` is the single source of truth.
